...
<head>
    ...
    <%
        ...
        //request: 요청 정보가 전달되는 객체 -> 요청한 정보가 해당 페이지를 벗어나면 소멸
        //response(반응): 결과를 돌려줄 페이지에 전달할 정보가 있는 객체 -> 요청한 페이지로 결과를 전달 후에 소멸
        //session: 로그인을 하고, 로그아웃 전까지 정보가 저장되는 객체 -> 로그아웃 시 소멸
        //application: 애플리케이션 정보가 담기는 객체로 브라우저가 닫히거나 웹 애플리케이션이 종료되면, 자동으로 소멸된다.
        //모든 웹의 객체는 attribute가 값을 저장할 수 있으며(setAttribute), 저장된 값을 가져올 수 있음(getAttribute)
...
<div id="msg">
    <%
        ...
        response.setHeader("Url2","http://github.com/emssme");
        response.setStatus(200); //성공처리
        //response.setStatus(400); //실패처리
        //response.setError(404, "요청하신 페이지가 존재하지않습니다.");
    %>
    ...
    <p> 저장된 상태 코드: <%=response.getStatus()%></p>
